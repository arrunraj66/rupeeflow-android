...
export default function App() { return <SafeAreaProvider><StatusBar style="light" /><Shell /></SafeAreaProvider>; }
function Shell() {
  const c = useLedger();
  ...
  return <ImageBackground source={require('./assets/finance-background.png')} imageStyle={{ opacity: c.state.profile.opacity }} style={{ flex: 1, backgroundColor: '#070C19' }}>
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={S.nav}>{tabs.map(([name, icon]) => <Pressable accessibilityRole="tab" accessibilityState={{ selected: name === tab }} key={name} onPress={() => setTab(name)} style={[S.navItem, name === tab && { backgroundColor: '#234259' }]}><Ionicons name={icon} size={23} color={name === tab ? C.cyan : C.muted} /><Text style={[S.navLabel, { color: name === tab ? C.cyan : C.muted }]}>{name}</Text></Pressable>)}</View>
