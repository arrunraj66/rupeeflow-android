...
<vector xmlns:android="http://schemas.android.com/apk/res/android" android:width="108dp" android:height="108dp" android:viewportWidth="108" android:viewportHeight="108">
  ...
  <path android:strokeColor="#5DEBFF" android:strokeWidth="5" android:strokeLineCap="round" android:fillColor="@android:color/transparent" android:pathData="M38,36 L68,36 M38,46 L68,46 M48,36 C67,36 66,55 40,55 L64,75"/>
...
module.exports = function withFinance(config) {
  config = withAndroidManifest(config, config => {
    ...
    app.receiver = (app.receiver || []).filter(x => x.$['android:name'] !== '.finance.FinanceSmsReceiver');
    app.receiver.push({ $: { 'android:name': '.finance.FinanceSmsReceiver', 'android:exported': 'true', 'android:permission': 'android.permission.BROADCAST_SMS' },
      'intent-filter': [{ action: [{ $: { 'android:name': 'android.provider.Telephony.SMS_RECEIVED' } }] }] });
    app.service = (app.service || []).filter(x => x.$['android:name'] !== '.finance.FinanceNotificationListener');
    app.service.push({ $: { 'android:name': '.finance.FinanceNotificationListener', 'android:label': 'RupeeFlow payment alerts', 'android:permission': 'android.permission.BIND_NOTIFICATION_LISTENER_SERVICE', 'android:exported': 'true' },
